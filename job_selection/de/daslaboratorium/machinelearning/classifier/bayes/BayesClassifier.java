package job_selection.de.daslaboratorium.machinelearning.classifier.bayes;
...
import job_selection.de.daslaboratorium.machinelearning.classifier.Classification;
...
public class BayesClassifier<T, K> extends Classifier<T, K> {
	...
	/**
	 * Retrieves a sorted <code>Set</code> of probabilities that the given set of
	 * features is classified as the available categories.
	 *
	 * @param features
	 *            The set of features to use.
	...
	private SortedSet<Classification<T, K>> categoryProbabilities(Collection<T> features) {
		...
		/*
		 * Sort the set according to the possibilities. Because we have to sort by the
		 * mapped value and not by the mapped key, we can not use a sorted tree
		 * (TreeMap) and we have to use a set-entry approach to achieve the desired
		 * functionality. A custom comparator is therefore needed.
		 */
